...
class Poolsteuerung extends utils.Adapter {
  ...
  calcPhDose(ph) {
    ...
    if (isNaN(delta) || delta <= Number(this.config.phMinDelta || 0)) return { ml:0, sec:0, delta };
    const base = Math.max(0, delta * 10 * Math.max(0, this.calcPoolVolumeL() / 100));
    const ml = Math.round(base * ((Number(this.config.phLiquidFactorPercent) || 100) / 100));
    const sec = Math.min(Number(this.config.phMaxRuntimeSec) || 300, Math.round((ml / Math.max(0.1, Number(this.config.phPumpFlowMlMin) || 16)) * 60));
    return { ml, sec, delta };
  }
  async phPreview() {
    const ph = await this.getForeignNumber(this.config.phStateId);
    if (ph === null) {
      this.debug('pH Vorschau: kein pH-Wert');
      return;
    }
    const r = this.calcPhDose(ph);
    this.debug(`pH Berechnung -> ph=${ph} delta=${r.delta} ml=${r.ml} sec=${r.sec}`);
    await this.setStateAsync('status.ph.previewMl', r.ml, true);
  ...
  async phDose(reason) {
    ...
    const r = this.calcPhDose(ph);
    if (r.ml <= 0 || r.sec <= 0) { this.debug('pH nicht nötig'); return; }
    this.phRunning = true;
    this.debug(`pH START -> reason=${reason} ml=${r.ml} sec=${r.sec}`);
    await this.setActorState(this.config.phPumpSocketStateId, true, 'pH START');
    await this.setStateAsync('status.ph.lastAction', `${reason}: gestartet`, true);
    setTimeout(async () => {
  ...
  async phTick() {
    if (!this.config.phEnabled || this.phRunning) {
      this.debug(`pH Tick übersprungen -> enabled=${this.config.phEnabled} running=${this.phRunning}`);
      return;
    }
    const now = this.hhmm();
    for (const t of this.parseTimes(this.config.phDoseTimes)) {
      if (t === now && !this.lastDoseKeys[t]) {
        this.lastDoseKeys[t] = Date.now();
        this.debug(`pH Automatik bei ${t}`);
        await this.phDose('Automatik ' + t);
      }
    }
  }
  async chlorTick() {
    if (!this.config.chlorinatorEnabled || !this.config.chlorinatorAutoEnabled) {
      this.debug('Chlorinator Tick übersprungen');
      return;
    }
