...
class Poolsteuerung extends utils.Adapter {
  ...
  buildTabletHtml(data) {
    const tempTimeLabels = Array.from({ length: 7 }, (_, i) => {
      const d = new Date(Date.now() - (24 - i * 4) * 60 * 60 * 1000);
...
.wrap{width:100%;max-width:1130px;height:740px;max-height:740px;padding:6px;overflow:hidden;margin:0 auto}
.layout{display:flex;gap:8px;align-items:flex-start;width:100%;height:728px;max-height:728px;overflow:hidden}
...
.temp-history-24h{position:relative;width:100%;height:46px;margin:2px 0 0;color:#39aef7;overflow:hidden;contain:paint;border-bottom:1px solid rgba(118,215,255,.10)}
.temp-history-24h .sparkline,.temp-history-24h svg{position:absolute;inset:0;width:100%!important;max-width:100%!important;height:46px!important;display:block;overflow:hidden!important;clip-path:inset(0)}
.temp-history-24h path{fill:none!important;stroke:#39aef7!important;stroke-width:2.8!important;stroke-linecap:round!important;stroke-linejoin:round!important;opacity:1!important}
.temp-history-24h circle{fill:#39aef7!important;opacity:1!important}
.temp-history-times{display:grid;grid-template-columns:repeat(7,1fr);gap:0;margin:1px 0 4px;font-size:8px;color:#9fc2df}.temp-history-times span{text-align:center}.temp-history-times span:first-child{text-align:left}.temp-history-times span:last-child{text-align:right}
...
.manual-presets-title{font-size:12px;color:#c8d7eb;font-weight:900;margin:1px 0 7px}
.manual-presets{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}
.manual-presets .manual-btn{min-height:50px;padding:7px 5px}
.manual-presets .manual-btn span{font-size:14px}
.manual-presets .manual-btn small{font-size:9px}
...
.ph-wa-flag{display:flex;align-items:center;gap:8px;margin-top:8px;padding:8px 10px;border-radius:11px;background:rgba(37,211,102,.10);border:1px solid rgba(37,211,102,.28);color:inherit;font-size:11px;font-weight:900;cursor:pointer;user-select:none}.ph-wa-flag input{width:19px;height:19px;margin:0;accent-color:#25d366;cursor:pointer;flex:0 0 auto}.ph-wa-flag span{line-height:1.15}</style></head><body><div class="wrap"><div class="layout">
  <div class="col-left">
    <div class="card hero">
      ...
      <div class="temp-history-24h">${data.poolTempSparklineSvg || ''}</div>
      <div class="temp-history-times">${tempTimeLabels.map(t => `<span>${esc(t)}</span>`).join('')}</div>
  ...
  <div class="col-mid">
    <div class="card">
      ...
      <div class="manual-presets-title">pH Manuell Dosierung</div>
      <div class="manual-presets">
        <button type="button" class="manual-btn js-manual-dose-btn" data-sec="60"><span>60 Sek.</span><small>Start Dosierung</small></button>
  ...
  <div class="col-right">
    ...
    <div class="card">
      ...
      <div class="mini-list">
        ...
        ${mini('pH zum Soll', `${data.phCorrectionText}\nGranulat: ${data.manualGranulateText}`, data.phCorrectionNeeded ? 'highlight' : 'info')}
...
.manual-btn{appearance:none;border:none;cursor:pointer;text-align:center;padding:6px 5px;border-radius:12px;min-height:48px;background:linear-gradient(180deg,#2d5da0 0%,#173765 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.18),0 6px 14px rgba(6,24,44,.22);border:1px solid rgba(255,255,255,.10);display:flex;flex-direction:column;justify-content:center;align-items:center;color:#fff;font-weight:900;gap:2px}
.manual-btn span{font-size:12px}.manual-btn small{font-size:8px;color:#dbeafe}.manual-dose-presets{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;grid-column:1 / -1;background:#fff;border:1px solid rgba(15,23,42,.08);border-radius:12px;padding:6px}.dose-drops{display:flex;align-items:center;justify-content:center;gap:1px;height:14px}.dose-drops svg{width:10px;height:13px;display:block;fill:none;stroke:#8fd8ff;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}
...
.ph-wa-flag{display:flex;align-items:center;gap:8px;margin-top:8px;padding:8px 10px;border-radius:11px;background:rgba(37,211,102,.10);border:1px solid rgba(37,211,102,.28);color:inherit;font-size:11px;font-weight:900;cursor:pointer;user-select:none}.ph-wa-flag input{width:19px;height:19px;margin:0;accent-color:#25d366;cursor:pointer;flex:0 0 auto}.ph-wa-flag span{line-height:1.15}</style>
  ...
  <div class="card"><div class="section-title">Schnellzugriff</div><div class="control-grid">
    ...
    <div class="manual-dose-presets">
      <button type="button" class="manual-btn js-manual-dose-btn" data-sec="60"><span>60 Sek.</span><span class="dose-drops"><svg viewBox="0 0 16 20" aria-hidden="true"><path d="M8 1.5S2.5 8.1 2.5 12.2A5.5 5.5 0 0 0 13.5 12.2C13.5 8.1 8 1.5 8 1.5Z"/></svg></span></button>
...
.ps-root{width:100%;max-width:1130px;height:740px;max-height:740px;overflow:hidden;padding:8px;margin:0 auto;color:#0f172a;font-family:Arial,Helvetica,sans-serif;background:linear-gradient(180deg,#0b1220 0%,#0f172a 100%)}
...
.ps-header{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.ps-title{font-size:15px;font-weight:800;color:inherit;white-space:nowrap;min-width:0}.ps-ver{font-size:8px;font-weight:800;color:#b9d7ff;margin-left:4px}.ps-sub{font-size:10px;color:#d4deec;text-align:right;white-space:nowrap}.ps-mode{display:inline-flex;align-items:center;justify-content:center;padding:3px 9px;border-radius:999px;border:1px solid rgba(255,255,255,.18);background:linear-gradient(180deg,#334f84,#1b3158);font-weight:800;font-size:11px;color:#fff;cursor:pointer}
.ps-tempRow{display:flex;align-items:center;gap:7px;margin:10px 0 4px;overflow:hidden;min-width:0}.ps-temp{font-size:70px;font-weight:900;line-height:.9;color:inherit;flex:0 0 auto}.ps-unit{font-size:20px;color:#d7e5f5;flex:0 0 auto;margin-top:24px}.ps-temp-inline-spark{position:relative;width:82px;flex:0 0 82px;height:36px;margin-left:1px;color:#76d7ff;overflow:hidden;contain:paint}.ps-temp-inline-spark .sparkline,.ps-temp-inline-spark svg{position:absolute;inset:0;width:82px!important;max-width:82px!important;height:36px!important;display:block;overflow:hidden;clip-path:inset(0)}
...
.ps-temp-spark{position:relative;height:44px;min-height:44px;margin:0;padding:0 2px;color:#39aef7;overflow:hidden;contain:paint;border-bottom:1px solid rgba(118,215,255,.12)}.ps-temp-spark .sparkline,.ps-temp-spark svg{position:absolute;left:2px;right:2px;top:0;width:calc(100% - 4px)!important;max-width:calc(100% - 4px)!important;height:44px!important;display:block;overflow:hidden;clip-path:inset(0)}.ps-temp-spark path{fill:none!important;stroke:#39aef7!important;stroke-width:2.8!important;stroke-linecap:round!important;stroke-linejoin:round!important;opacity:1!important}.ps-temp-spark circle{fill:#39aef7!important;stroke:none!important;opacity:1!important}.ps-temp-times{display:grid;grid-template-columns:repeat(7,1fr);font-size:7px;color:#b6cce3;margin:1px 2px 4px}.ps-temp-times span{text-align:center}.ps-temp-times span:first-child{text-align:left}.ps-temp-times span:last-child{text-align:right}.ps-metrics{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:auto}.ps-metric{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:10px;min-height:94px;overflow:hidden;contain:paint}.ps-k{font-size:12px;color:inherit;opacity:.88;margin-bottom:6px;font-weight:700}.ps-kline{display:flex;align-items:center;height:30px;margin-bottom:2px;overflow:hidden;min-width:0}.ps-kline .ps-k{margin:0;white-space:nowrap;flex:0 0 auto}.ps-k-spark{position:relative;width:72px;flex:0 0 72px;height:26px;margin-left:auto;overflow:hidden;contain:paint}.ps-k-spark .sparkline,.ps-k-spark svg{position:absolute;inset:0;width:72px!important;max-width:72px!important;height:26px!important;display:block;overflow:hidden;clip-path:inset(0)}.ps-v{font-size:26px;font-weight:900;line-height:1.1;color:#0f172a;display:flex;align-items:center;min-width:0}.ps-v.ps-wrap{font-size:13px;font-weight:700;line-height:1.2;word-break:break-word;overflow-wrap:anywhere;white-space:normal}.ps-s{font-size:11px;color:#e3edf9;margin-top:6px}.ps-hero .ps-v{color:#fff}.ps-chip{display:inline-flex;align-items:center;justify-content:center;padding:3px 8px;border-radius:999px;font-size:9px;font-weight:800;margin-top:6px}.ps-chip.good{background:#dcfce7;color:#166534}.ps-chip.warn{background:#fef3c7;color:#92400e}.ps-chip.bad{background:#fee2e2;color:#991b1b}.ps-chip.neutral{background:#e2e8f0;color:#334155}
.ps-block-title{font-size:16px;font-weight:800;color:#0f172a;margin-bottom:8px}.ps-list{display:grid;gap:6px}.ps-row{display:grid;grid-template-columns:minmax(88px,116px) minmax(0,1fr);gap:8px;align-items:start;background:#ffffff;border:1px solid rgba(15,23,42,.08);border-radius:14px;padding:8px}.ps-actions-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}.ps-action-btn{appearance:none;border:none;cursor:pointer;text-align:left;padding:10px 12px;border-radius:14px;min-height:58px;background:linear-gradient(180deg,#2d4f86 0%,#162d52 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.15),0 8px 18px rgba(6,24,44,.28);border:1px solid rgba(255,255,255,.09);display:flex;flex-direction:column;justify-content:center;gap:4px}.ps-action-btn:disabled{opacity:.5;cursor:default}.ps-action-name{font-size:14px;font-weight:800}.ps-action-state{font-size:12px;font-weight:800}.ps-action-btn.is-on .ps-action-name,.ps-action-btn.is-on .ps-action-state{color:#67dd7c}.ps-action-btn.is-off .ps-action-name,.ps-action-btn.is-off .ps-action-state{color:#ff8d7b}.ps-statuswrap{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}.ps-sparkline-wrap{display:inline-flex;align-items:center;margin-left:10px;flex:1;min-width:0;max-width:100%;overflow:hidden}.sparkline{width:100%;height:34px;display:block;overflow:hidden}.sparkline path{fill:none;stroke:currentColor;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round;opacity:.98}.sparkline circle{fill:currentColor;opacity:.98}.sparkline-ph{color:#48b8ff}.sparkline-orp{color:#5be878}.sparkline-temp{color:#76d7ff}
...
<div class="ps-root"><div class="ps-grid">
  <div class="ps-card ps-hero">
    ...
    <div class="ps-temp-times">${tempTimeLabels.map(t => `<span>${esc(t)}</span>`).join('')}</div>
...
.ps-temp-spark{height:28px;margin:2px 0 6px;color:#76d7ff}.ps-temp-spark .sparkline{width:100%;height:28px;display:block;overflow:visible}.sparkline-temp{color:#76d7ff}
...
(function(){
  ...
  async renderVisFull(force = false) {
    ...
    try {
      await this.updateLocalTrendHistory(ph, orp, poolTemp);
    } catch (e) {
    ...
    // Bei aktiver Waage wird der Nettowert ausschließlich zentral in
    // updatePhCanisterFromScale() berechnet. Alle Anzeigen und Warnungen lesen
    // danach denselben State status.phCanister.netWeightKg.
    if (phCanCfg.scaleEnabled) await this.updatePhCanisterFromScale(false);
    ...
    if (phCanCfg.scaleEnabled) {
      ...
      phCanUnit = 'kg netto';
      phCanSource = `${phCanCfg.weightStateId} − ${phCanCfg.tareKg.toFixed(3)} kg Tara`;
    }
    ...
    let historySparklines = { phSparklineSvg: '', orpSparklineSvg: '', poolTempSparklineSvg: '' };
    try {
      ...
      historySparklines = { phSparklineSvg: '', orpSparklineSvg: '', poolTempSparklineSvg: '' };
    }
    if (!historySparklines.phSparklineSvg || !historySparklines.orpSparklineSvg || !historySparklines.poolTempSparklineSvg) {
      try {
        ...
        historySparklines.poolTempSparklineSvg = historySparklines.poolTempSparklineSvg || localSparklines.poolTempSparklineSvg || '';
      } catch (e) {
    ...
    const poolTempNumForSparkline = parseNum(poolTemp);
  ...
  buildSparklineSvgFromValues(values, cssClass = '', timeMode = 'today') {
    ...
    const strokeColor = cssClass === 'sparkline-temp' ? '#76d7ff' : (cssClass === 'sparkline-orp' ? '#5be878' : '#48b8ff');
    return `<svg class="sparkline ${cssClass}" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-hidden="true" focusable="false"><path d="${path}" fill="none" stroke="${strokeColor}" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"></path><circle cx="${r(last.x)}" cy="${r(last.y)}" r="1.8" fill="${strokeColor}"></circle></svg>`;
  }
  ...
  async ensureLocalTrendHistoryStates() {
    ...
    await this.ensureState('status.trend.poolTemp24hJson', 'string', 'json', '[]', false);
  }

  async updateLocalTrendHistory(phValue, orpValue, poolTempValue) {
    ...
    const tempNum = parseNum(poolTempValue);
    if (Number.isFinite(tempNum)) {
      let tempArr = [];
      try {
        const raw = await this.getText('poolsteuerung.0.status.trend.poolTemp24hJson', '[]');
        const parsed = JSON.parse(raw || '[]');
        if (Array.isArray(parsed)) tempArr = parsed;
      } catch (e) { tempArr = []; }
      const last = tempArr.length ? tempArr[tempArr.length - 1] : null;
      if (!last || (now - Number(last.ts || 0)) >= 300000 || Math.abs(Number(last.val) - tempNum) >= 0.05) {
        tempArr.push({ ts: now, val: Math.round(tempNum * 100) / 100 });
      }
      const cutoff = now - 24 * 60 * 60 * 1000;
      tempArr = tempArr.filter(x => Number(x.ts) >= cutoff && Number.isFinite(Number(x.val))).slice(-288);
      await this.setStateIfChanged('status.trend.poolTemp24hJson', JSON.stringify(tempArr), true);
    }
  }
  ...
  async getLocalTrendSparklines() {
    ...
    const [phValues, orpValues, poolTempValues] = await Promise.all([
      ...
      readArr('status.trend.poolTemp24hJson')
    ]);
    return {
      ...
      poolTempSparklineSvg: this.buildSparklineSvgFromValues(poolTempValues, 'sparkline-temp', '24h')
    };
  }
  ...
  async onReady() {
    try {
      this.log.info('[VIS] v0.4.0 Diagnose-Logging aktiv');
