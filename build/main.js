...
@media (max-width:1100px){
  ...
  buildPhoneHtml(data) {
    ...
    const metricValue = (value, trend = '→', ok = false) => `<span class="metric-main ${ok ? 'ok' : ''}">${esc(value)}</span><span class="trend ${trendClass(trend)} ${ok ? 'ok' : ''}" style="margin-left:30px;font-weight:900;">${esc(trend)}</span>`;
...
(function(){
  ...
  buildTabletWidget(data) {
    ...
    const trendClass = trend => trend === '↑' ? 'up' : (trend === '↓' ? 'down' : 'flat');
    const metricValue = (value, trend = '→', ok = false) => `<span class="ps-mmain ${ok ? 'ok' : ''}">${esc(value)}</span><span class="ps-trend ${trendClass(trend)} ${ok ? 'ok' : ''}" style="margin-left:30px;font-weight:900;">${esc(trend)}</span>`;
    return `
...
.ps-metric{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:6px}.ps-ml{font-size:10px;color:#d9e5f5}.ps-mv{font-size:13px;font-weight:900;color:#fff;display:flex;align-items:center}.ps-ms{display:none}.ps-mmain.ok{color:#67dd7c}.ps-trend{font-size:14px;font-weight:900;color:#c9d7ee;line-height:1;display:inline-flex;min-width:14px;justify-content:center;margin-left:30px}.ps-trend.up{color:#ffb36b}.ps-trend.down{color:#7dd3fc}.ps-trend.flat{color:#c9d7ee}.ps-trend.ok{color:#67dd7c}.ps-section{font-size:12px;font-weight:900;color:#0f172a;margin-bottom:3px}
