...
class Poolsteuerung extends utils.Adapter {
  ...
  buildTabletHtml(data) {
    const mkButton = (id, value, label) => {
      if (!id) return '';
      return `<button class="ctlBtn" onclick="setIo('${esc(id)}', ${value})">${label}</button>`;
    };

    const status = [
      `<div class="statusItem">
        <div>
          <div class="statusName">Umwälzpumpe</div>
          <div class="statusHint">Grundlauf / Zeitfenster</div>
          <div class="btnRow">${mkButton(data.pumpId, !data.pumpOn, data.pumpOn ? 'Ausschalten' : 'Einschalten')}</div>
        </div>
        <div class="pill ${data.pumpOn ? 'on' : 'off'}">${data.pumpOn ? 'EIN' : 'AUS'}</div>
      </div>`,
      `<div class="statusItem">
        <div>
          <div class="statusName">Chlorinator</div>
          <div class="statusHint">ORP-Regelung</div>
          <div class="btnRow">${mkButton(data.chlorId, !data.chlorOn, data.chlorOn ? 'Ausschalten' : 'Einschalten')}</div>
        </div>
        <div class="pill ${data.chlorOn ? 'on' : 'off'}">${data.chlorOn ? 'EIN' : 'AUS'}</div>
      </div>`,
      `<div class="statusItem">
        <div>
          <div class="statusName">pH-Dosierpumpe</div>
          <div class="statusHint">Automatik / manuell</div>
          <div class="btnRow">${mkButton(data.phPumpId, !data.phPumpOn, data.phPumpOn ? 'Ausschalten' : 'Einschalten')}</div>
        </div>
        <div class="pill ${data.phPumpOn ? 'on' : 'off'}">${data.phPumpOn ? 'EIN' : 'AUS'}</div>
      </div>`,
      `<div class="statusItem">
        <div>
          <div class="statusName">Wärmepumpe</div>
          <div class="statusHint">Solar / Batterie</div>
          <div class="btnRow">${mkButton(data.heatpumpId, !data.heatpumpOn, data.heatpumpOn ? 'Ausschalten' : 'Einschalten')}</div>
        </div>
        <div class="pill ${data.heatpumpOn ? 'on' : 'off'}">${data.heatpumpOn ? 'EIN' : 'AUS'}</div>
      </div>`
    ].join('');
...
<style>
:root{
  --bg:#08111c;--panel:#0f1b2d;--panel2:#13243a;--line:rgba(255,255,255,.08);--line2:rgba(255,255,255,.05);
  --text:#f8fafc;--muted:#97abc5;--accent:#58c9ff;--accentSoft:rgba(88,201,255,.14);
  --ok:#22c55e;--ok2:#34d399;--off:#ef4444;--off2:#f87171;--shadow:0 14px 34px rgba(0,0,0,.34)
}
*{box-sizing:border-box}
body{margin:0;color:var(--text);font-family:Arial,Helvetica,sans-serif;background:
radial-gradient(circle at top left, rgba(38,110,185,.22) 0%, rgba(8,17,28,0) 34%),
linear-gradient(180deg,#091321 0%,#08111c 100%)}
.wrap{width:980px;height:730px;padding:16px;overflow:hidden}
.grid{display:grid;grid-template-columns:400px 250px 282px;gap:16px;height:100%}
.card{display:flex;flex-direction:column;min-width:0;background:linear-gradient(180deg,rgba(15,27,45,.98),rgba(19,36,58,.98));
border:1px solid var(--line);border-radius:24px;padding:18px;box-shadow:var(--shadow)}
.heroHeader{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}
.title{font-size:20px;font-weight:700;letter-spacing:.2px}
.sub{font-size:12px;color:var(--muted);text-align:right;white-space:nowrap}
.badge{display:inline-flex;align-items:center;gap:8px;width:max-content;margin-top:12px;padding:8px 12px;border-radius:999px;
background:var(--accentSoft);border:1px solid rgba(88,201,255,.22);color:#8fdcff;font-size:12px}
.tempRow{display:flex;align-items:flex-end;gap:8px;margin:18px 0 14px}
.tempMain{font-size:96px;font-weight:800;line-height:.88;letter-spacing:-2px}
...
.metric{background:linear-gradient(180deg,rgba(255,255,255,.035),rgba(255,255,255,.02));border:1px solid var(--line2);border-radius:18px;padding:14px;min-height:104px}
.metric .k{font-size:13px;color:var(--muted);margin-bottom:10px}
.metric .v{font-size:24px;font-weight:700}
.energyList,.statusWrap{display:grid;gap:10px;margin-top:12px}
.energyRow,.statusItem{display:grid;grid-template-columns:1fr auto;align-items:center;background:linear-gradient(180deg,rgba(255,255,255,.035),rgba(255,255,255,.02));
border:1px solid var(--line2);border-radius:18px;padding:12px 14px;min-width:0}
.energyRow{min-height:58px}
.statusItem{min-height:86px}
.energyRow .k,.statusHint{font-size:12px;color:var(--muted)}
.energyRow .v{font-size:18px;font-weight:700;text-align:right;max-width:126px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.on{background:linear-gradient(180deg,var(--ok2),var(--ok))}.off{background:linear-gradient(180deg,var(--off2),var(--off))}
.btnRow{margin-top:10px}
.ctlBtn{background:rgba(255,255,255,.06);color:#dbeafe;border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:7px 10px;font-size:12px;font-weight:700;cursor:pointer}
.ctlBtn:hover{background:rgba(255,255,255,.10)}
.smallIcon{opacity:.95;margin-right:6px}
</style>
<script>
function setIo(id, value){
  try{
    if(window.parent && window.parent.vis && typeof window.parent.vis.setValue === 'function'){
      window.parent.vis.setValue(id, value);
      return;
    }
    if(window.vis && typeof window.vis.setValue === 'function'){
      window.vis.setValue(id, value);
      return;
    }
    console.log('VIS setValue nicht verfügbar', id, value);
  }catch(e){ console.log(e); }
}
</script></head><body><div class="wrap"><div class="grid">
<div class="card">
  ...
  <div class="tempRow"><div class="tempMain">${esc(data.poolTemp)}</div><div class="unit">°C</div></div>
  <div class="metricGrid">
    ...
    <div class="metric"><div class="k"><span class="smallIcon">🌡</span>Außen</div><div class="v">${esc(data.outsideTemp)}°C</div></div>
    <div class="metric"><div class="k"><span class="smallIcon">🎯</span>Soll</div><div class="v">${esc(data.targetTemp)}°C</div></div>
  </div>
</div>
<div class="card">
...
<div class="card">
  ...
  async renderVis() {
    ...
    const data = {
      ...
      pumpId: this.config.circulationPumpSocketStateId || '',
      chlorId: this.config.chlorinatorSocketStateId || '',
      phPumpId: this.config.phPumpSocketStateId || '',
      heatpumpId: this.config.heatpumpPowerStateId || '',
    };
