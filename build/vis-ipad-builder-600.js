...
function chart(rows, current, color, digits, minRange, maxPoints) {
  ...
  return {
    svg: `<svg class="history-svg" viewBox="0 0 460 105" preserveAspectRatio="none"><path class="grid" d="M0 27H460M0 53H460M0 79H460"/><path d="${path}" fill="none" stroke="${color}" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/><circle cx="${round(x(last.ts))}" cy="${round(y(last.val))}" r="2" fill="${color}"/></svg>`,
...
function canisterHtml(data) {
  ...
  const netKg = numberValue(canister.netWeightKg ?? canister.netKg ?? canister.weightKg ?? canister.currentWeightKg);
...
function buildHtml(data, history, maxPoints) {
  ...
  const heating = heatpumpOn && /(heiz|heat|warm)/i.test(String(data.heatpumpMode ?? ''));
  ...
  return `<style data-ipad-single-owner="1">*{box-sizing:border-box}.ipad-final{position:fixed;inset:0;width:100vw;height:100vh;z-index:2147483000;padding:8px 10px;display:grid;grid-template-rows:44px minmax(0,1fr) 38px 38px;gap:7px;overflow:hidden;background:radial-gradient(circle at 10% -10%,rgba(53,145,230,.20),transparent 35%),linear-gradient(145deg,#06101c,#0a1a2c 58%,#07131f);color:#f6fbff;font-family:"Segoe UI Variable","Segoe UI",Arial,sans-serif}.ipad-final header{display:flex;align-items:center;justify-content:space-between;padding:0 4px}.brand{display:flex;align-items:center;gap:9px}.brand-mark{width:29px;height:29px;border-radius:9px;display:grid;place-items:center;background:linear-gradient(145deg,#228bd8,#28c8c0);font-size:16px}.brand-title{display:flex;align-items:center;gap:10px;font-size:21px;font-weight:760;letter-spacing:.08em}.brand-sub,.meta{font-size:9px;color:#9bb0c8;font-weight:700}.meta{text-align:right;line-height:1.3}.pump{display:inline-flex;align-items:center;gap:6px;padding:4px 8px;border:1px solid rgba(255,255,255,.12);border-radius:999px;background:rgba(255,255,255,.055);font-size:8px}.pump i,.device i{width:10px;height:10px;border-radius:50%;background:#77899c}.pump.on i,.device.on i{background:#62e27c;box-shadow:0 0 10px rgba(98,226,124,.65)}.pump:not(.on) i{background:#ff655c;box-shadow:0 0 9px rgba(255,101,92,.45)}.cards{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:7px;min-height:0}.metric-card{position:relative;min-width:0;min-height:0;overflow:hidden;border:1px solid rgba(255,255,255,.10);border-radius:16px;padding:10px 14px 8px;background:linear-gradient(150deg,rgba(17,40,65,.98),rgba(8,24,41,.98));display:grid;grid-template-rows:30px 78px 30px minmax(70px,1fr) 18px}.metric-card:after{content:"";position:absolute;left:0;right:0;bottom:0;height:4px;background:var(--accent)}.metric-head{display:grid;grid-template-columns:28px 1fr auto;align-items:center;gap:8px}.metric-icon{width:28px;height:28px;border-radius:9px;display:grid;place-items:center;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.08);color:var(--accent)}.metric-label{font-size:17px;font-weight:700}.metric-period{font-size:8px;color:#9bb0c8;font-weight:800;text-transform:uppercase}.metric-reading{display:flex;align-items:center;justify-content:center;gap:7px;white-space:nowrap}.metric-value{font-size:70px;font-weight:760;line-height:.88;letter-spacing:-.055em;color:var(--accent)}.metric-unit{font-size:24px;font-weight:650;color:rgba(247,251,255,.8)}.metric-trend{font-size:27px;font-weight:650;color:#afbed0}.card-controls{display:flex;align-items:center;justify-content:space-between;gap:7px;min-width:0}.dose-buttons{display:flex;gap:5px}.dose-buttons button{width:72px;height:32px;padding:2px 4px;border:1px solid rgba(255,255,255,.12);border-radius:9px;background:linear-gradient(180deg,#2d4f86,#162d52);color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer}.dose-buttons b{font-size:10px;line-height:10px}.dose-buttons small{font-size:7px;line-height:7px;color:#dbeafe}.device{display:inline-flex;align-items:center;gap:6px;height:21px;padding:3px 7px;border:1px solid rgba(255,255,255,.11);border-radius:999px;background:rgba(4,17,30,.82);font-size:8px;font-weight:900;color:#aab9c8;white-space:nowrap}.device.on{color:#a8f4b7;border-color:rgba(98,226,124,.25);background:rgba(37,110,55,.26)}.history-wrap{min-height:0;border-radius:9px;overflow:hidden;background:rgba(3,14,25,.3)}.history-svg{width:100%;height:100%;display:block}.grid{fill:none;stroke:rgba(255,255,255,.07);stroke-width:1}.history-empty{height:100%;display:grid;place-items:center;color:#9bb0c8;font-size:11px}.history-meta{display:flex;align-items:center;justify-content:space-between;color:#9bb0c8;font-size:9px;font-weight:750}.canister{min-width:164px;padding:5px 10px;border:1px solid rgba(164,124,255,.22);border-radius:8px;background:rgba(4,17,30,.84);text-align:right}.canister b,.canister small{display:block;white-space:nowrap}.canister b{font-size:12px;line-height:1.15}.canister small{font-size:10px;line-height:1.15;color:#b7c9dc;margin-top:2px}.canister.ok b{color:#7cea91}.canister.warn b{color:#ffd06e}.canister.critical b{color:#ff756d}.heat,.schedule{border:1px solid rgba(255,255,255,.10);border-radius:12px;background:rgba(10,28,47,.96);align-items:center;padding:5px 9px}.heat{display:grid;grid-template-columns:1.3fr repeat(3,1fr);gap:5px}.heat>div{display:flex;justify-content:space-between;gap:6px;padding:0 8px;border-left:1px solid rgba(255,255,255,.07);font-size:9px;color:#9bb0c8}.heat>div:first-child{border-left:0;color:#eaf4ff;font-size:11px;font-weight:900}.heat b{font-size:11px;color:#dbe9f6}.heat b.on{color:#67df7e}.schedule{display:grid;grid-template-columns:auto 1fr;gap:9px;overflow:hidden}.schedule-label{font-size:9px;color:#9bb0c8;font-weight:900;text-transform:uppercase}.schedule-list{display:flex;justify-content:flex-end;gap:5px;overflow:hidden}.schedule-chip{max-width:32%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;border:1px solid rgba(84,200,255,.17);background:rgba(84,200,255,.09);border-radius:999px;padding:4px 7px;font-size:8px;color:#deedff;font-weight:750}.schedule-empty{font-size:9px;color:#9bb0c8}</style><main class="ipad-final"><header><div class="brand"><span class="brand-mark">◉</span><div><div class="brand-title">POOL<span class="pump ${pumpOn ? 'on' : ''}"><i></i>Umwälzpumpe ${pumpOn ? 'EIN' : 'AUS'}</span></div><div class="brand-sub">24H LIVE-VERLAUF</div></div></div><div class="meta"><div>${esc(data.updated || '--')}</div><div>${VERSION} · iPad Mini · 1024 × 768</div></div></header><section class="cards">${cardHtml}</section><section class="heat"><div>↻ Wärmepumpe</div><div><span>LÄUFT</span><b class="${heatpumpOn ? 'on' : ''}">${heatpumpOn ? 'JA' : 'NEIN'}</b></div><div><span>HEIZT</span><b class="${heating ? 'on' : ''}">${heating ? 'JA' : 'NEIN'}</b></div><div><span>DREHZAHL</span><b>${fan === null ? '--' : `${Math.round(fan)} %`}</b></div></section><footer class="schedule"><div class="schedule-label">Nächste Schaltungen</div><div class="schedule-list">${scheduleHtml(data.nextActionsText)}</div></footer></main><script data-ipad-single-owner="1">(function(){var ns=${namespace};function api(){try{if(window.vis)return window.vis}catch(e){}try{if(window.parent&&window.parent.vis)return window.parent.vis}catch(e){}try{if(window.top&&window.top.vis)return window.top.vis}catch(e){}return null}async function setState(id,value){var v=api();try{if(v&&typeof v.setValue==='function'){var r=v.setValue(id,value);if(r&&typeof r.then==='function')await r;return true}}catch(e){}try{if(v&&v.conn&&typeof v.conn.setState==='function'){var q=v.conn.setState(id,value);if(q&&q.then==='function')await q;return true}}catch(e){}return false}document.addEventListener('click',async function(event){var button=event.target&&event.target.closest?event.target.closest('[data-dose]'):null;if(!button||button.disabled)return;event.preventDefault();event.stopPropagation();var old=button.innerHTML;button.disabled=true;button.innerHTML='<b>…</b><small>Start Dosierung</small>';var seconds=Number(button.dataset.dose)||60;var ok=await setState(ns+'.control.ph.manualDoseSec',seconds);if(ok)ok=await setState(ns+'.control.ph.manualTrigger',Date.now());button.innerHTML=ok?'<b>OK</b><small>Dosierung gestartet</small>':'<b>Fehler</b><small>nicht gestartet</small>';setTimeout(function(){button.innerHTML=old;button.disabled=false},1400)},true)})();</script>`;
}
...
async function buildIpadMini(adapter, data) {
  const history = await loadHistory(adapter);
  for (const points of [28, 22, 16, 12]) {
    const html = buildHtml({ ...(data || {}), namespace: adapter.namespace }, history, points);
    if (Buffer.byteLength(html, 'utf8') <= MAX_HTML_BYTES) return html;
  }
  throw new Error('canonical iPad Mini HTML exceeds size limit');
}

module.exports = { buildIpadMini };
