...
function install(adapter) {
  ...
  // Niemals einen numerischen Ersatzwert an die VIS weiterreichen, solange
  // 0.5.34 den autoritativen pH-Pfad noch nicht erfolgreich synchronisiert hat.
  ...
  // Vor jedem VIS-Render muss 0.5.34 zuerst synchronisieren. Danach wird erneut
  // geprüft. Ist der Pfad noch nicht valide, wird der Render komplett verworfen,
  // statt einen falschen pH-Wert in HTML/Widget zu schreiben.
  ...
  // Sicherheitsgurt für die Regelung: Solange der echte Sensor beim Start noch
  // nicht verfügbar ist, darf keine pH-Dosierentscheidung auf einem Fremdstate
  // oder Altwert basieren.
  ...
  // Wenn PH803W später verfügbar wird, Startschutz automatisch freigeben und
  // einen frischen VIS-Render anstoßen. Damit ist kein manueller Browser-Reload
  // als Reparaturmaßnahme mehr nötig.
  adapter.on('stateChange', (id, state) => {
    ...
    const handle = adapter.trackTimeout(setTimeout(async () => {
      ...
      // 0.5.34 verarbeitet dieselbe State-Änderung und aktualisiert zunächst den
      // autoritativen Wert. Kurze Verzögerung verhindert eine Listener-Race.
