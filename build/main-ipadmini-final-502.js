...
const STYLE = `<style data-pool-update-502="1">
...
function makeButton(namespace, info) {
  ...
  return `<button type="button" class="pool-update-btn ${cssClass}" data-pool-update-068="1" data-update-502="1" data-available="${available ? 1 : 0}" data-running="${running ? 1 : 0}" data-target="${escapeAttribute(info.availableVersion || '')}" title="${escapeAttribute(title)}" onclick="${clickHandler(namespace)}"${running ? ' disabled' : ''}>${escapeHtml(text)}</button>`;
}
...
function install(adapter) {
  ...
  async function writeInfo(changes) {
    Object.assign(adapter.__update502Info, changes);
    const info = adapter.__update502Info;
    await adapter.setStateAsync(IDS.installed, CURRENT, true);
  ...
  function schedulePatch() {
    if (adapter.__update502PatchTimer || adapter.isShuttingDown) return;
    const timer = setTimeout(async () => {
      adapter.__update502PatchTimer = null;
      try { await patchStates(); } catch {}
    }, 100);
    adapter.__update502PatchTimer = timer;
  ...
  async function check(reason) {
    ...
    try {
      ...
      await writeInfo({
        ...
        available,
        status: available ? `Update verfügbar: ${CURRENT} → ${remote}` : `Version ${CURRENT} ist aktuell`,
        error: '',
        lastCheck: Date.now()
      });
  ...
  for (const name of ['buildTabletHtml', 'buildTabletWidget', 'buildPhoneHtml', 'buildPhoneWidget']) {
    ...
    adapter[name] = data => patchButtonView(original({ ...(data || {}), adapterVersion: VERSION }), adapter.namespace, adapter.__update502Info);
  }
