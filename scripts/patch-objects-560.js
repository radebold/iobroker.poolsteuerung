...
function patchFile(rel,replacements){
  ...
  if(!fs.existsSync(file)){console.warn(`[0.5.60] Datei fehlt: ${rel}`);return;}
  let src=fs.readFileSync(file,'utf8');
  let changed=0;
  for(const [from,to] of replacements){
    if(src.includes(from)){src=src.split(from).join(to);changed++;}
  }
  if(changed){fs.writeFileSync(file,src);console.log(`[0.5.60] ${rel}: ${changed} normale Diagnose-Logs auf info gesetzt`);}
  else console.log(`[0.5.60] ${rel}: keine Aenderung erforderlich`);
}

patchFile('build/main-ipadmini-final-517.js',[
  ["adapter.log.warn('[CHLOR-OWNER] v0.5.17 aktiv:","adapter.log.info('[CHLOR-OWNER] v0.5.17 aktiv:"],
  ["adapter.log.warn(`[CHLOR-OWNER 0.5.17] AUS-Befehl blockiert", "adapter.log.info(`[CHLOR-OWNER 0.5.17] AUS-Befehl blockiert"],
  ["adapter.log.warn(`[CHLOR-OWNER 0.5.17] Chlorinator unerwartet AUS; EIN wiederhergestellt", "adapter.log.info(`[CHLOR-OWNER 0.5.17] Chlorinator unerwartet AUS; EIN wiederhergestellt"]
]);
patchFile('build/main-ipadmini-final-524.js',[
  ["adapter.log.warn(`[CHLOR-HARDGUARD 0.5.24] ZbSend Power:0 blockiert", "adapter.log.info(`[CHLOR-HARDGUARD 0.5.24] ZbSend Power:0 blockiert"]
]);
patchFile('build/main-ipadmini-final-525.js',[
  ["adapter.log.warn(`[CHLOR-HARDGUARD 0.5.25] Bootstrap aktiv", "adapter.log.info(`[CHLOR-HARDGUARD 0.5.25] Bootstrap aktiv"]
]);
